...
type Btc15mScreenProps = {
  addToast: AddToast;
};

export function Btc15mScreen({ addToast }: Btc15mScreenProps) {
  const [status, setStatus] = useState<Btc15mStatusPayload | null>(null);
  ...
  const trades: Btc15mCompletedTrade[] = status?.dryRun
  ...
  async function toggleBot() {
    ...
    setLoading(true);
    try {
      const payload = await toggleBtc15mBotRequest(isActive, formConfig);
      setStatus(payload);
      addToast(
        "success",
